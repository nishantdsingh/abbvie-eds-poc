...
function buildLinzessIconImageCardColumn(wrapper, columnIndex) {
  ...
  const ctaLabel = (ctaP?.textContent?.trim() || ctaDiv?.textContent?.trim() || '');
  const hasCta = ctaLabel.length > 0;
  const titleP = titleDiv?.querySelector('p');
  ...
  if (bodyDiv) {
    [...bodyDiv.children].forEach((node) => {
      if (!['P', 'UL', 'OL'].includes(node.tagName)) return;
      const clone = node.cloneNode(true);
      if (clone.tagName === 'P') {
        clone.classList.add('text-align-center');
        fixLinzessEncodedBoldInParagraph(clone);
        fixEncodedSupInParagraph(clone);
      } else {
        clone.querySelectorAll('p').forEach((p) => {
